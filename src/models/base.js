...
Model.prototype.getAssociationInfo = function*(modelA, modelB, data) {
  ...
  if (data instanceof Array) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (idsA.indexOf(data[i].id) !== -1) continue;
      idsA.push(data[i].id);
    }
  } else {
    var idsA = [data.id];
...
Model.prototype.getAssociationData = function*(model, associationInfo) {
  ...
  var ids = [];
  for (var i = 0; i < associationInfo.length; i++) {
    if (ids.indexOf(associationInfo[i].id) !== -1) continue;
    ids.push(associationInfo[i][id]);
  }
    ...
    for (var i = 0; i < associationData.length; i++) {
      for (var j = 0; j < associationInfo.length; j++) {
        if (item.id === associationInfo[j][idA] && associationData[i].id === associationInfo[j][idB]) {
          ...
          item[modelB + 's'][item[modelB + 's'].length - 1].associations = {};

          for (var k in associationInfo[j]) {
            if (k === 'id') continue;
            if (k === idA) continue;
            if (k === idB) continue;
            item[modelB + 's'][item[modelB + 's'].length - 1].associations[k] = associationInfo[j][k];
          }
